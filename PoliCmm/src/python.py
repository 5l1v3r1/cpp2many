...
class Python (formatter.Formatter):
    ...
    def vProgram (self, node):
        self.p('# Translated to Python with PoliCmm')
        self.p('import plib,sys,math')
        self.p('sys.setrecursionlimit(50000)')
        self.p('M_PI = 3.1415926535897932384626433')
        self.p()
        for c in node.children:
            self.visit(c)      
        self.p('_w_ = plib.Wrapper(None)')
        self.p('plib.readinput()')
        for i in range(10):
            self.p('_par%d_ = plib.Stack()' % i)
        self.p();
        self.p('main( )')
    ...
    def vCinStatement (self, node):
        ...
        if node.is_expression:
            ...
            for c in node.children:
                ...
                tpe = c.resolveType()
                #print tpe.type
                self.w('(plib.checkread%s(_w_) and ([' % tpe.type)
                self.w(c.id)
                self.w('for')
                self.w(c.id)
                self.w('in [_w_.value]]))')
            ...
            for c in node.children:
                ...
                if tpe.isInt():
                    self.w('plib.readint()')
                if tpe.isFloat():
                    self.w('plib.readdouble()')
                if tpe.isDouble():
                    self.w('plib.readdouble()')
                if tpe.isString():
                    self.w('plib.readstring()')
                if tpe.isChar():
                    self.w('plib.readchar()')
                if tpe.isBool():
                    self.w('plib.readint() != 0')
    ...
    def vCoutElement (self,node):
        if node.element.resolveType().isFloatingPoint():
            self.w('sys.stdout.write (plib.precisionstring % (')
            self.visit(node.element)
            self.w('))')
        elif node.element.resolveType().isBool():
            self.w('if')
            self.visit(node.element)
            self.pi(':')
            self.pu("sys.stdout.write('1')")
            self.pi('else:')
            self.pu("sys.stdout.write('0')")
        else:
            formatstring = {'int': '%d', 'string': '%s', 'char' : '%c'}
            self.w('sys.stdout.write(\'%s\' %% (' % formatstring[node.element.resolveType().type])
            self.visit(node.element)
            self.w('))')
            
    def vCoutBreakLine (self,node):
        self.p('sys.stdout.write(\'\\n\')')
        
        
    def vCoutModifier (self, node):
        if node.func == 'precision':
            self.w('plib.setprecision(')
    ...
    def vAssignmentStatement (self, node):
        if isinstance(node.lval, ast.Reference) and node.lval.value.resolveType().isString():
            ...
            self.w('=')
            self.w('plib.setcharat(')
            self.visit(node.lval.value)
            self.w(',')
            self.visit(node.lval.idx)
            self.w(',')
            if not node.expr.resolveType().isChar():
                self.w('chr(')
            if node.operator == '+=':
                self.w('ord(')
    ...
    def initializeVector(self, node, tpe):
        if tpe.isVector():
            self.w('[')
            if len(node.params.children) > 1:
                self.w('')
                self.initializeVector(node.params.children[1], tpe.subtype)
                self.w('')
            else:
                self.initializeVector(None, tpe.subtype)
            self.w('for _i_ in range(')
    ...
    def vVariableDeclaration (self, node):
        ...
        if tpe.isVector() and node.params:
            ...
            if tpe.isVector() and node.params:
                self.w(' = ')
                self.initializeVector(node, tpe)
            return
            ...
            #self.visit(node.params.children[0])
            #self.w(']')
        if tpe.isString() and node.params:
            self.w('=')
        ...
        if node.init:
            ...
            self.w('=')
            self.initializeType(node.type.resolveType())
            self.p()
    ...
    def vIdentifier (self, node):
        ...
        if node.resolveType().is_reference:
            self.w('.value')
        #if hasattr(node, 'use_reference') and node.use_reference:
            #self.w(node.id)
        #else:
            #self.w(node.id)
            #self.w('.value')
    ...
    def vLiteral (self, node):
        if node.resolveType().isString() or node.resolveType().isChar():
            self.w('"' + node.lit + '"')
        elif node.resolveType().isBool():
            self.w(self.t[node.lit])
        else:
            self.w(str(node.lit))
    ...
    def vBinaryOp (self, node):
        ...
        if (left_tpe.isChar() or right_tpe.isChar())  and not (node.oper =='<=' or node.oper == '<' or node.oper == '>=' or node.oper == '>' or node.oper == '==' or node.oper == '!='):
            ...
            self.w(self.t[node.resolveType().type])
            self.w('(')
            if left_tpe.isChar():
                self.w('ord(')
    ...
    def vFunctionCall (self, node):
        ...
        if node.isSortCall:
            ...
            if len(node.params.children) > 2:
                self.cmpfunc_id += 1
                self.pi('def cmpfunc%d(x,y):' % (self.cmpfunc_id))
                self.pi('if %s(plib.Var(x),plib.Var(y)):' % node.sortComp)
                self.pu('return -1')
                self.pi('if %s(plib.Var(y),plib.Var(x)):' % node.sortComp)
                self.pu('return 1')
                self.pu('return 0')
            ...
            if len(node.params.children) > 2:
                self.w(',')
                self.w('cmp = cmpfunc%d' % (self.cmpfunc_id))
            self.w(')')
            return
        ...
        for i in range(len(node.params.children)):
            ...
            if node.type.params[i].is_reference:
                self.w('_par%d_.push(plib.Var(' % i)
        ...
        if has_ref:
            ...
            for i in range(len(node.type.params)):
                if node.type.params[i].is_reference and not isinstance(node.params.children[i], ast.FunctionCall):
                    if not first:
                        if rettype.type != 'void':
                            self.w('and')
                        else:
                    ...
                    if node.type.params[i].resolveType().isChar() and isinstance(node.params.children[i], ast.Reference) and node.params.children[i].value.resolveType().isString():
                        ...
                        self.w('for')
                        self.visit(node.params.children[i].value)
                        self.w(' in [ plib.setcharat(')
                        self.visit(node.params.children[i].value)
                        self.w(',')
                        self.visit(node.params.children[i].idx)
                        self.w(',')
                        self.w('_par%d_.pop().value' % i)
                        self.w(') ] ]')
                    else:
                        self.w('[')
                        self.visit(node.params.children[i])
        ...
        #for i in range(len(node.params.children)):
            ...
            #else: self.w(',')
            #if ftype.params[i].is_reference:
                #node.params.children[i].use_reference = True
                #self.visit(node.params.children[i])
            #else:
                #self.w('plib.Var(')
                #self.visit(node.params.children[i])
    ...
    def vUnaryOp (self, node):
        if node.right.resolveType().isChar():
            if isinstance(node.right, ast.Reference) and node.right.value.resolveType().isString():
                self.w('[')
    ...
    def vConstructor (self, node):
        ...
        elif node.resolveType().isVector():

            self.initializeVector(node, node.type.resolveType())
    ...
    def vCastExpression (self, node):
        if node.type.resolveType().isBasic():
            self.w(self.t[node.type.type])
            self.w('(')
